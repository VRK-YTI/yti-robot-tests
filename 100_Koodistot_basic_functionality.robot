...
108. Check User right management link
    ...
    Click element    ${USER_RIGHT_MANAGEMENT}
    Sleep    2
    Run Keyword If    "${ENVIRONMENT_URL}" == "https://koodistot-test.suomi.fi/"    Select Window    title=TEST - Reference Data
    ...    ELSE    Select Window    title=DEV - Reference Data
    [Teardown]    Restore Finnish language
...
112. Check navigation menu links
    ...
    Run Keyword If    "${ENVIRONMENT_URL}" == "https://koodistot-test.suomi.fi/"    Select Window    title=TEST - Tietomallit
